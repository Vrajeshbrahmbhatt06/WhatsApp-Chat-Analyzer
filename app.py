...
if uploaded_file is not None:
    ...
    date_list = df['only_date'].unique().tolist()
    date_list.insert(0, "All Dates")
    date = st.sidebar.selectbox('Date Input', date_list)
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        if emoji_df.empty is True:
            ...
            with col2:
                ...
                ax.pie(emoji_df[1].head(), labels=emoji_df[0].head(), autopct="%0.2f", textprops=prop)
                st.pyplot(fig)

        # sentiment analysis
        sentiment_df = helper.sentiment_analysis(selected_user, df, date)
        ...
        with col2:
            ...
            fig, ax = plt.subplots()
            ax.barh(sentiment_counts.index, sentiment_counts.values)

            # Set titles and labels
            ax.set_title('Number of Messages per Sentiment')
        ...
        st.title("Message Summarization")
        summary = helper.summarization(selected_user, df, date)
        if summary == "No messages on the selected date":
            st.write(summary)
        else:
            st.markdown(summary)